...
var LevelConfig = {
    ...
    getExpToNextLevel(currentLevel) {
        ...
        const currentExp = this.getExpForLevel(currentLevel);
        const nextExp = this.getExpForLevel(currentLevel + 1);
     ...
     */
    getExpProgress(totalExp) {
        const currentLevel = this.getLevelFromExp(totalExp);
     ...
     */
    getStatBonus(level, statType) {
        if (level <= 1) return 0;

        // 基础属性加成倍数（每级增加的百分比）
        const bonusMultipliers = {
            hp: 0.05,      // 每级增加5%最大生命值
            attack: 0.03,  // 每级增加3%攻击力
            defense: 0.03, // 每级增加3%防御力
            speed: 0.02,   // 每级增加2%速度
            crit: 0.01,    // 每级增加1%暴击率
            miss: 0.01     // 每级增加1%闪避率
        };

        const multiplier = bonusMultipliers[statType] || 0;
        // 返回累计加成（相对于1级的倍数）
     ...
     */
    calculateStatValue(baseStat, level, statType) {
        if (level <= 1) return baseStat;
        const bonus = this.getStatBonus(level, statType);
