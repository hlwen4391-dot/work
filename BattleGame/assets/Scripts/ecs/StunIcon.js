...
cc.Class({
    ...
    showStun() {
        ...
        const graphics = this.stunIcon.addComponent(cc.Graphics);
    ...
    hideStun() {
        if (this.stunIcon && this.stunIcon.isValid) {
            // 淡出动画
            cc.tween(this.stunIcon)
                .to(0.2, { opacity: 0, scale: 0.5 })
                .call(() => {
                    if (this.stunIcon && this.stunIcon.isValid) {
     ...
     * 绘制眩晕圆圈效果（多个圆圈旋转）
     */
    _drawStunCircles(graphics, radius) {
        // 外圈 - 鲜艳的亮黄色大圆圈
        ...
        const lineCount = 8; // 8条旋转线
        for (let i = 0; i < lineCount; i++) {
            const angle = (Math.PI * 2 / lineCount) * i;
            const startX = Math.cos(angle) * radius * 0.4;
    ...
    _startAnimation() {
        ...
        // 旋转动画（圆圈旋转效果）
        cc.tween(this.stunIcon)
            .by(1.5, { angle: 360 })  // 1.5秒转一圈，更快更明显
            .union()
            .repeatForever()
            .start();
        ...
        // 上下浮动动画
        cc.tween(this.stunIcon)
            .to(0.5, { y: originalY + 5 }, { easing: 'sineInOut' })
            .to(0.5, { y: originalY - 5 }, { easing: 'sineInOut' })
            .union()
            .repeatForever()
            .start();
